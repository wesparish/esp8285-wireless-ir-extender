...
Wireless IR extender using two ESP8285 (ESP-01M) modules with integrated IR transceivers. One **media-node** sits in the media room near the projector and receives IR signals from the AVR remote. It relays them over ESP-NOW to the **server-node**, which re-transmits the IR signal in front of the AVR receiver in the server room. The result is a seamless remote control experience with the AVR physically relocated away from the media room.
...
./bin/flash <media-node|server-node> [port]   # validate config, build, flash
...
- `media-node`: `-D NODE_ROLE=ROLE_MEDIA` — enables IR receive, disables IR transmit
- `server-node`: `-D NODE_ROLE=ROLE_SERVER` — enables IR transmit, disables IR receive
...
Both nodes call `WiFi.mode(WIFI_STA)` and `wifi_set_channel()` but never join a network. The media-node is `ESP_NOW_ROLE_CONTROLLER`; the server-node is `ESP_NOW_ROLE_SLAVE`.
